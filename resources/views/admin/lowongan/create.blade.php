        ...
        <div class="card-body">
            <form action="{{ route('admin.jobs.store') }}" method="POST" enctype="multipart/form-data" id="form-name">
                ...
                <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                    ...
                    <input type="text" id="id" name="id" value="{{ $mitra->id }}" class="d-none">
                    <input type="text" id="title" name="title" class="form-control" value="{{ $mitra->posisi }}"
                        required>
                    @if ($errors->has('title'))
                ...
                <div class="form-group {{ $errors->has('location_id') ? 'has-error' : '' }}">
                    ...
                    <select name="location_id" id="location" class="form-control select2" required>
                        @foreach ($locations as $id => $location)
                            <option value="{{ $id }}"
                                {{ (isset($job) && $job->location ? $job->location->id : old('location_id')) == $id ? 'selected' : '' }}>
                                {{ $location }}</option>
                        @endforeach
                    </select>
                    ...
                    <label for="new_location" class="mt-3">Kota Baru (Masukkan jika kota tidak ada di
                        list)</label>
                ...
                <div class="form-group {{ $errors->has('address') ? 'has-error' : '' }}">
                    ...
                    <input type="text" id="address" name="address" class="form-control"
                        value="{{ old('address', isset($job) ? $job->address : '') }}">
                ...
                <div class="form-group {{ $errors->has('full_description') ? 'has-error' : '' }}">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered" id="dynamic_field2">
                            <tr>
                                <td><input id="full_description" name="full_description[]" type="text"
                                        placeholder="Masukkan Deskripsi Pekerjaan" class="form-control name_list" /></td>
                                <td><button type="button" name="add2" id="add2" class="btn btn-success">Add More</button>
                                </td>
                            </tr>
                        </table>
                    </div>
                ...
                <div class="form-group {{ $errors->has('requirements') ? 'has-error' : '' }}">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered" id="dynamic_field">
                            <tr>
                                <td><input id="requirements" name="requirements[]" type="text"
                                        placeholder="Masukkan Syarat Pekerjaan" class="form-control name_list" /></td>
                ...
                <div class="form-group {{ $errors->has('bataslamaran') ? 'has-error' : '' }}">
                    ...
                    <input type="text" id="bataslamaran" name="bataslamaran" class="form-control"
                        value="{{ $mitra->bataslamaran }}">
                ...
                <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                    ...
                    <input type="text" id="email" name="email" class="form-control" value="{{ $mitra->email }}"
                        required>
                    @if ($errors->has('email'))
                        <em class="invalid-feedback">
                            {{ $errors->first('email') }}
                ...
                <div class="form-group {{ $errors->has('top_rated') ? 'has-error' : '' }}">
                    ...
                    <input value="1" type="checkbox" id="top_rated" name="top_rated"
                        {{ old('top_rated', 0) == 1 ? 'checked' : '' }}>
